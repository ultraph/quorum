...
function fmtTok(o){
  ...
  if(!ti && !to) return '';
  return ` · in ${ti.toLocaleString()} · out ${to.toLocaleString()} tokens${o.tokens_est?' (~est)':''}`;
}
  ...
  while(true){
    ...
    while((i=buf.indexOf('\n\n'))>=0){
      ...
      if(o.type==='model'){
        ...
        const tk=(tokIn||tokOut) ? ` <span class="meta">· total ${(tokIn+tokOut).toLocaleString()} tokens `
